puts "inside LibDs280e01.tcl File" ; update
# ds28e01
...
proc GetPageFile {} {
  ...
  set gaSet(FileVer) 1   
  set gaSet(IdTyp) 2  
###   set val "[set gaSet(chbPageDbr)][set gaSet(chbPageApl)]"
###   switch $val {
###     00 {  
###           return Ok
###     }
###     01 {  
###           set fileName "$gaSet(Page)"
###     }
###     10 {  
###           exec [pwd]\\Get28e01Data.exe  $gaSet(IdTyp) $gaSet(barcode) $gaSet(FileVer)
###           Delay_Sec 3
###           set fileName "$gaSet(barcode).txt"     
###     }
###     default {
###       puts stderr "Syntax Err ... (proc GetPageFile)"
###       set gaGui(Perfm) "Syntax Err ..." ; update 
###       return "Abort"     
###     }
###   }

  set fileName "$gaSet(pageFile)"
  ...
  ###if {$gaSet(chbPageDbr)==1} {file delete $fileName} 
...
# Write
# ***************************************************************************
proc WritePages {chs mc mode} {
  global gaGui gaGet buffer buff  gaSet
  set com $gaSet(comMC.$chs.$mc)
  
	if {[Send $com "\r" "\[boot" 1] != 0} {
	  set gaSet(fail) "Chassis-$chs MC-$mc. Communication fail." ; update
    puts stderr "Chassis-$chs MC-$mc. Communication fail." 
    return -1
	}
  ...
  if {$ret!=1} {
    puts stderr "Chassis-$chs MC-$mc. Device IP obtention fail." 
    return -1	  
  }
  ...
  if {$gaSet(PageSetupType)=="AllPagesWithBarcode"} {
	...
	for {} {$page <=3} {incr page 1} {
		#set crcInf [exec ds01crc.exe $gaGet(page$page)]
		#set crc [lindex $crcInf 1]
		set device 00 ; #constant
		set crc [CRC $gaGet(page$page)]
		set offSet 00   
    Status "Chassis-$chs MC-$mc. Writing page $page in $mode"   	
		
		#Write:
    if {[Send $com "c2 $device,0$page,$offSet,$gaGet(page$page),$crc\r" "data ?" 3] != 0} {
		  set gaSet(fail) "Chassis-$chs MC-$mc [string toupper $mode]. Writing Error - page $page"
      puts stderr "Chassis-$chs MC-$mc [string toupper $mode]. Writing Error - page $page" 
      return -1
    }			      
    ...
    if {$ret!=1} {
      set gaSet(fail) "Chassis-$chs MC-$mc [string toupper $mode]. Page$page check fail." ; update
      #puts stderr "Page$page check fail." 
      return -1	  
    }
    ...
    if {[string match *$gaGet(page$page)* $res]==0} {
      set gaSet(fail) "Chassis-$chs MC-$mc [string toupper $mode]. Page$page result fail." ; update
      puts "Chassis-$chs MC-$mc [string toupper $mode]. res:$res"
      puts "Chassis-$chs MC-$mc [string toupper $mode]. pag:$gaGet(page$page)"
