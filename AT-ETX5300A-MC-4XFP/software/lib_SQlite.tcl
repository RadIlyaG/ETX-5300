...
# SQliteAddLine
# ***************************************************************************
proc SQliteAddLine {ba} {
  global gaSet
  
  set barcode $gaSet(barcode${ba})
  puts "SQliteAddLine $barcode"
  if {[string match *skip* $barcode]} {
    ## do not include skipped in stats
    return 0
  }
  if {$gaSet(barcode${ba}.IdMacLink)=="link"} {
    ## do not report about passed unit
    # 10:36 04/10/2023 return 0
  }
  set uut "ETX-5300/MC" ; #$gaSet(DutFullName)
...
#   
#   for {set tr 1} {$tr <= 6} {incr tr} {
#     if [catch {gaSet(dataBase) eval {INSERT INTO tbl VALUES($barcode,$uut,$hostDescription,$date,$tim,$status,$failTestsList,$failReason,$operator0)}} res] {
#       set res "Try${tr}_fail.$res"
