...
#  package require ezsmtp
  ezsmtp::config -mailhost EXRAD-IL.ad.rad.co.il -from "ATE-ETX5300A-MC-4XFP"
  ...
  for {set i 1} {$i<=$gaSet(EmailSum)} {incr i} {   
    if {$gaSet(chbutEmail.$i)==1} {
      ...
      -subject "[string toupper [info host] ] : Message from Tester $gaSet(tester)" \
      -body "\n[string toupper [info host] ] : Message from Tester $gaSet(tester)\n\
       \n$msg " \
